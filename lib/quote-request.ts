...
const URGENCY_MATCHERS = [
  {
    value: "Emergency / ASAP",
    keywords: ["emergency", "urgent", "asap", "now", "today", "immediately"],
  },
...
function findExactOption(raw: unknown, options: readonly string[]) {
  const value = normalizeText(raw);
  return options.find((option) => normalizeText(option) === value) || "";
}

function findByKeywords(raw: unknown, matchers: ReadonlyArray<{ value: string; keywords: readonly string[] }>) {
  const value = normalizeText(raw);
  if (!value) return "";
...
export function buildQuoteWhatsAppMessage(form: QuoteForm, { source = "website" } = {}) {
  const lines = [
...
export function buildQuotePrefillSearch(form: QuoteForm, { source = "assistant" } = {}) {
...
export function readQuotePrefill(search = ""): QuotePrefill {
