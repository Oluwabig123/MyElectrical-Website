...
export function pickBlogImage({
  blueprint,
  generatedPost,
  recentPosts = [],
  requestedTopic = "",
}: {
  blueprint?: { category?: string } | null;
  generatedPost?: {
    title?: string;
    summary?: string;
    points?: string[];
  ...
  recentPosts?: Array<{ image_url?: string | null }>;
  ...
  const textCorpus = normalizeText(
    [
      ...
      ...((generatedPost?.sections || []).flatMap((section) => [
        section.heading,
        ...(section.body || []),
      ])),
    ].join(" "),
  ...
  const imageUsageCounts = recentImageUrls.reduce<Record<string, number>>((counts, imageUrl) => {
  ...
  const rankedImages = BLOG_IMAGE_GALLERY.map((image) => {
    let score = 0;
    const categoryMatch = image.categories.some(
      (category) => category === String(blueprint?.category || ""),
    );
